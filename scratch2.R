...
state="TX"
year="2021"
path1=paste0("data2/", state, "_", year, "_01.csv")
path2=paste0("data2/", state, "_", year, "_02.csv")
path3=paste0("data2/", state, "_", year, "_03.csv")
path4=paste0("data2/", state, "_", year, "_04.csv")
...
d4 <- read_csv(path4) %>% 
  ...
  right_join(d3) %>% 
  mutate(year = year) %>% 
  select(year, county, county_code, annual_county_population, annual_all_deaths, annual_hospital_deaths, month, monthly_hospital_deaths, covid_monthly_hospital_deaths)
